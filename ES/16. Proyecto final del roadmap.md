<!-- COURSE_NAV_START -->
[Anterior](15.%20Profesionalización%20por%20rol.md) | [Indice](README.md) | [Siguiente](17.%20GitOps%20y%20delivery%20continuo%20con%20Argo%20CD.md.md)
<!-- COURSE_NAV_END -->
...
## 16.22 Simulacro CKAD del proyecto final

Este simulacro no sustituye el proyecto final.

Sirve para comprobar si puedes operar bajo presión.

Tiempo recomendado: 45 minutos.

Reglas:

- No uses apuntes internos del curso.
- Puedes usar documentación oficial de Kubernetes.
- Trabaja solo con terminal.
- No copies manifests completos si puedes generarlos.
- Valida cada resultado.

### Tarea 1. Namespace y contexto

Crea el namespace `shop-ckad` y configúralo como namespace actual.
...
debe mostrar `shop-ckad` como namespace.

### Tarea 2. Deployment

Crea un Deployment `checkout-api` con 3 réplicas, imagen `nginx:1.27`, puerto 80 y labels `app=checkout-api`.
...
Expón el Deployment con un Service ClusterIP llamado `checkout-api` que publique el puerto 80.
...
### Tarea 4. ConfigMap y Secret

Crea un ConfigMap `checkout-api-config` con `LOG_LEVEL=info`.

Crea un Secret `checkout-api-secret` con `API_TOKEN=demo`.

Móntalos en el Deployment como variables de entorno.
...
Añade readinessProbe y livenessProbe HTTP contra `/`.
...
Añade requests de CPU y memoria, y limit de memoria.
...
Actualiza la imagen a `nginx:1.28`.
...
Crea un Ingress para `checkout.local` que apunte al Service `checkout-api`.
...
### Tarea 9. NetworkPolicy

Crea una NetworkPolicy default deny ingress.

Después permite tráfico hacia Pods `app=checkout-api` desde Pods con label `role=frontend`.
...
Rompe el Deployment cambiando la imagen a una inexistente.

Diagnostica el error.

Repara el Deployment.
...
El simulacro está superado si puedes:

- Crear recursos rápido.
- Inspeccionarlos sin perderte.
- Diagnosticar fallos.
- Reparar el sistema.
...
## 16.25. Qué faltaría para producción real
...
## 16.26. Criterio de salida del módulo
...
## 16.27. Referencias oficiales y fuentes primarias
