<!-- COURSE_NAV_START -->
[Anterior](2.%20Por%20qué%20aparece%20Kubernetes.md) | [Indice](README.md) | [Siguiente](4.%20Modelo%20mental%20de%20Kubernetes.md)
<!-- COURSE_NAV_END -->
...
## 3.3 bis. Fluidez `kubectl` para CKAD

CKAD no mide si sabes leer una explicación larga.

Mide si puedes operar Kubernetes con precisión desde la terminal.

Eso exige fluidez con `kubectl`.

No necesitas memorizar todo Kubernetes, pero sí necesitas dominar los movimientos básicos.
...
Este curso usará `kubectl` en los ejemplos para ser explícito, pero en práctica de examen puedes usar `k`.

### Namespace por defecto

En CKAD es habitual trabajar en namespaces específicos.

Puedes evitar repetir `-n shop` en cada comando configurando el namespace del contexto actual:
...
### Crear YAML sin crear el recurso
...
El objetivo no es usar imperativo en producción.

El objetivo es generar un punto de partida rápido y después editarlo.

### Comandos que debes automatizar mentalmente
...
Antes de buscar en documentación, prueba:
...
> En CKAD, `kubectl` no es una herramienta auxiliar. Es el medio principal para pensar, crear, inspeccionar, corregir y validar.
...
sequenceDiagram
  ...
  Kubectl->>API: Envía objeto Pod
  API->>Etcd: Guarda estado deseado
  Kubelet->>API: Observa Pods asignados al nodo
  Kubelet->>Runtime: Crea contenedor
  Runtime-->>Kubelet: Estado del contenedor
